...
function renderTable(bookings) {
  ...
  tbody.innerHTML = bookings.map(b => {
    ...
    var showQueue = (b.status === 'accepted' || b.status === 'pending') && b.queue_position;
    var queueDisplay = showQueue ? ' — #' + b.queue_position : '';
...
async function markArrived(id) {
  try {
    ...
    // Also handle old pending bookings as accepted
    if (daySnap.exists()) {
      daySnap.forEach(function(child) {
        var v = child.val();
        if (v.status === 'pending' && child.key !== id) {
          accepted.push({ id: child.key, queue: v.queue_position || 999 });
        }
      });
    }
    ...
    if (oldDate !== newDate) {
      ...
      if (oldDaySnap.exists()) {
        oldDaySnap.forEach(function(child) {
          var v = child.val();
          if ((v.status === 'accepted' || v.status === 'pending') && child.key !== id) {
            accepted.push({ id: child.key, queue: v.queue_position || 999 });
          }
        });
      }
  ...
  try {
    ...
    if (dateStr) {
      ...
      if (daySnap.exists()) {
        daySnap.forEach(function(child) {
          var v = child.val();
          if (v.status === 'accepted' || v.status === 'pending') {
            accepted.push({ id: child.key, queue: v.queue_position || 999 });
          }
        });
      }
