# PROJECT MAP — Dr. Ruhm Badr Booking System (v4)
...
**`/bookings/{pushId}`**
- `booking_number` (number) — max 11 digits (YYMMDD + 5rand)
- `patient_name` (string)
- `phone` (string)
- `service` (string) — selected from 5 cards
- `preferred_date` (string) — YYYY-MM-DD
- `status` (string) — "accepted" | "arrived" | "cancelled" | "rejected"
- `queue_position` (number) — position within same date only
...
| Booking number max 11 digits (YYMMDD + 5rand) | ✅ Done | `app.js` |
| Auto-accept on booking (no pending state) | ✅ Done | `app.js` (status: 'accepted' immediately) |
| Today-first day search | ✅ Done | `app.js` (i=0 loop) |
| Queue position per-day only | ✅ Done | `app.js`, `dashboard/dashboard.js` |
| Success view with booking number + date | ✅ Done | `index.html`, `app.js` |
| Status lookup page (shows queue + date) | ✅ Done | `status.html`, `app.js` |
| Real-time dashboard (RTDB on('value')) | ✅ Done | `dashboard/dashboard.js` |
| Cancel booking (doctor side) | ✅ Done | `dashboard/dashboard.js` |
| Mark arrived (حضر) — queue auto-shifts | ✅ Done | `dashboard/dashboard.js` |
| Queue only shows for `accepted` (not arrived/cancelled) | ✅ Done | `app.js`, `dashboard.js` |
| Copy booking number in dashboard table | ✅ Done | `dashboard/dashboard.js` |
...
| Connection badge (dashboard only) | ✅ Done | `firebase/config.js` |
| Tenvora credit strip | ✅ Done | `index.html` |
