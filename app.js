...
function showSuccess(number, preferredDate) {
  ...
  if (preferredDate) {
    ...
    document.getElementById('successDate').textContent = '📅 موعدك المتوقع: ' + dateStr;
  }
...
async function createBooking(data) {
  ...
  if (phoneSnap.exists()) {
    phoneSnap.forEach(function(child) {
      ...
      if (st === 'pending' || st === 'accepted') hasActive = true;
    });
  }
  if (hasActive) {
  ...
  for (let i = 1; i <= 31; i++) {
    ...
    if (daySnap.exists()) {
      daySnap.forEach(function(child) {
        var st = child.val().status;
        if (st === 'pending' || st === 'accepted') activeCount++;
      });
    }
  ...
  // Generate unique booking number from timestamp
  const now = new Date();
  ...
  const bookingNumber = parseInt(y + m + d + h + min + rand);
...
if (statusForm) {
  statusForm.addEventListener('submit', async function(e) {
    ...
    var statusText = { pending: 'قيد الانتظار', accepted: 'تم القبول', arrived: 'حضر', rejected: 'مرفوض' };
    ...
    var queueStr = data.queue_position ? '<div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border-light);"><span style="color:var(--text-muted);font-size:0.85rem;">ترتيبك في الطابور</span><span style="font-weight:700;color:var(--gold);font-size:1.1rem;">#' + data.queue_position + '</span></div>' : '';
    ...
    resultDiv.innerHTML = '<div style="background:var(--surface);border-radius:var(--radius-md);padding:32px;margin-top:20px;border:1px solid var(--border-light);animation:fade-up 0.5s ease-out;">' +
      ...
      (data.status === 'rejected' && data.rejection_reason ? '<div style="margin-top:8px;padding:16px;background:#fef2f2;border-radius:var(--radius-sm);border:1px solid #fecaca;"><p style="font-weight:600;color:#991b1b;margin-bottom:4px;font-size:0.85rem;">سبب الرفض:</p><p style="color:#b91c1c;font-size:0.9rem;">' + escapeHtml(data.rejection_reason) + '</p></div>' : '') +
      '</div></div>';
  });
}
